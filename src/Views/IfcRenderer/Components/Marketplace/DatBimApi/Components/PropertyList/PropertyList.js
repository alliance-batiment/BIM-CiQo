...
const PropertyList = ({
  ...
  function addProperties(properties, objSelected) {
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/createPset/${projectId}`, {
      ...
      axios.post(process.env.REACT_APP_API_URL, formData).then((value) => {
        ...
        axios.get(`${process.env.REACT_APP_API_URL}/p/${projectId}`).then(() => {
          history.push(`/temp`);
          history.goBack();
          setLoader(false);
        });
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {properties?.map((property, propertyIndex) => (
                <TableRow key={property.property_id} className={`${classes.root} ${classes.datBimList}`}>
                  <TableCell width="40%" component="th" scope="row">
                    {property.property_name}
                  </TableCell>
                  <TableCell width="40%" align="right">
                    {DefineTypeComponent(property.data_type_name, property, propertyIndex, configureProperty)}
                  </TableCell>
