...
const PortalList = ({ classes, openObjects, setSelectedPortal, handleNext, setActiveStep }) => {
  ...
  return (
    <Grid item xs={12}>
      {
        portals?.map((portal) => <Card key={portal.portal_id} className={`${classes.root} ${classes.datBimCard}`}>
          <CardContent onClick={() => {
